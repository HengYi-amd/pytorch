...
"""FlyDSL gfx950 GEMM kernels for FP16 and BF16."""
...
def gemm_gfx950_kernel(
    ...
    for k_tile in range(0, main_loop_end, 1):
        ...
        waitcnt_barrier((stages - 2) * ldg_wait_count)
    ...
    for s in range_constexpr(0, stages - 1):
        waitcnt_barrier((stages - 2 - s) * ldg_wait_count)
...
def gemm_hti_gfx950_kernel(
    ...
    waitcnt_barrier(half_ldg_b_iters + half_ldg_a_iters)
    ...
    def compute_double_tile(k_tile, prefetch_next):
        ...
        if const_expr(prefetch_next):
            ...
            waitcnt_barrier(2 * half_ldg_b_iters + half_ldg_a_iters)
